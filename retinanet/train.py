...
def train(
    ...
    optimizer = SGD(
        model.parameters(), lr=lr, weight_decay=regularization_l2, momentum=0.9
    )
    ...
    while iteration < iterations:
        ...
        for i, (data, target) in enumerate(data_iterator):
            ...
            if is_master and (profiler.totals["train"] > 60 or iteration == iterations):
                ...
                if verbose:
                    msg = "[{:{len}}/{}]".format(
                        iteration, iterations, len=len(str(iterations))
                    )
                    msg += " focal loss: {:.3f}".format(focal_loss)
                    msg += ", box loss: {:.3f}".format(box_loss)
                    msg += ", {:.3f}s/{}-batch".format(
                        profiler.means["train"], batch_size
                    )
            ...
            if val_annotations and (
                iteration == iterations or iteration % val_iterations == 0
            ):
                infer(
                    model,
                    val_path,
            ...
            if (iteration == iterations and not rotated_bbox) or (
                iteration > iterations and rotated_bbox
            ):
                break
