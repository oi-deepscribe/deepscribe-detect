...
def generate_anchors_rotated(stride, ratio_vals, scales_vals, angles_vals):
    ...
    widths_heights = widths_heights.repeat(
        int(theta.size(0) / widths_heights.size(0)), 1
    )
    ...
    anchors_rotated = order_points(torch.stack([xy0R, xy1R, xy2R, xy3R], dim=1)).view(
        -1, 8
    )
...
def snap_to_anchors(boxes, size, stride, anchors, num_classes, device, anchor_ious):
    ...
    depth[overlap >= anchor_ious[1]] = (
        classes[indices][overlap >= anchor_ious[1]].squeeze() + 1
    )  # objects
...
def snap_to_anchors_rotated(
    boxes, size, stride, anchors, num_classes, device, anchor_ious
):
    ...
    anchors_axis = (
        (xy_2corners.to(torch.float) + anchors_axis.view(-1, 1, 1, 4))
        .contiguous()
        .view(-1, 4)
    )
    anchors_rotated = (
        (xy_4corners.to(torch.float) + anchors_rotated.view(-1, 1, 1, 8))
        .contiguous()
        .view(-1, 8)
    )
    ...
    overlap = iou(
        boxes_rotated.contiguous().view(-1), anchors_rotated.contiguous().view(-1)
    )[0]
    ...
    # Generate target classes
    cls_target = torch.zeros(
        (anchors_axis.size()[0], num_classes + 1), device=device, dtype=boxes_axis.dtype
    )
    if classes.nelement() == 0:
        classes = torch.LongTensor([num_classes], device=device).expand_as(indices)
    else:
...
def nms_rotated(all_scores, all_boxes, all_classes, nms=0.5, ndetections=100):
    ...
    for batch in range(batch_size):
        ...
        # Sort boxes
        scores, indices = torch.sort(scores, descending=True)
        boxes, boxes_theta, classes = (
            boxes[indices],
            boxes_theta[indices],
            classes[indices],
        )
        areas = (boxes_theta[:, 2] - boxes_theta[:, 0] + 1) * (
            boxes_theta[:, 3] - boxes_theta[:, 1] + 1
        ).view(-1)
        keep = torch.ones(scores.nelement(), device=device, dtype=torch.uint8).view(-1)
        ...
        for i in range(ndetections):
            ...
            overlap, inter = iou(
                boxes_rotated.contiguous().view(-1),
