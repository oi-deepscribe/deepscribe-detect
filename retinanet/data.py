...
class CocoDataset(data.dataset.Dataset):
    ...
    def collate_fn(self, batch):
        ...
        if self.training:
            ...
            targets = [
                torch.cat([t, torch.ones([max_det - t.size()[0], 5]) * -1])
                for t in targets
            ]
...
class DataIterator:
    ...
    def __init__(
        ...
        self.sampler = (
            data.distributed.DistributedSampler(self.dataset) if world > 1 else None
        )
...
class RotatedCocoDataset(data.dataset.Dataset):
    ...
    def _get_target(self, id):
        ...
        for ann in annotations:
            ...
            assert len(ann["bbox"]) == 5, (
                "Bounding box for id %i does not contain five entries." % id
            )
    ...
    def collate_fn(self, batch):
        ...
        if self.training:
            ...
            max_det = max([t.size()[0] for t in targets])
            targets = [
                torch.cat([t, torch.ones([max_det - t.size()[0], 6]) * -1])
                for t in targets
            ]
            targets = torch.stack(targets, 0)
        else:
